...
import train.common.core.network.PacketDEGActivate;
...
public class GuiDEG extends GuiContainer {

    private LiquidTank liquid;
    private EntityPlayer player;
    public StandardDEGCar degMinecart;
    public GuiDEG(EntityPlayer player, InventoryPlayer inventoryplayer, Entity entityminecart) {
        super(new InventoryLiquid(inventoryplayer, (LiquidTank) entityminecart));
        System.out.println("Created?");
        liquid = (LiquidTank) entityminecart;
        this.player = player;
        this.degMinecart = ((StandardDEGCar)entityminecart);
    }
    ...
    public void initGui() {
        ...
        if (degMinecart.isOn) {
            this.buttonList.add(new GuiButton(4, var1 + 40, var2 - 10, 70, 10, "Stop Engine"));
        } else {
            this.buttonList.add(new GuiButton(4, var1 + 40, var2 - 10, 70, 10, "Start Engine"));
        }
    }
    @Override
    protected void actionPerformed(GuiButton guibutton) {
        if (guibutton.id == 3) {
            ...
            if(player!=null && player.getCommandSenderName().toLowerCase().equals(((AbstractTrains) liquid).getTrainOwner().toLowerCase())) {
                if ((!liquid.getTrainLockedFromPacket())) {
                    if (degMinecart.isOn) {
                        degMinecart.isOn = false;
                        guibutton.displayString = "Start Engine";
                        Traincraft.degActivateChannel.sendToServer(new PacketDEGActivate(0, degMinecart.getEntityId()));
                    } else {
                        degMinecart.isActivated = true;
                        guibutton.displayString = "Stop Engine";
                        Traincraft.degActivateChannel.sendToServer(new PacketDEGActivate(1, degMinecart.getEntityId()));
                    }
                    this.initGui();
                    System.out.println(degMinecart.isActivated);
                }
            } else if(player!=null && player instanceof EntityPlayer){
                player.addChatMessage(new ChatComponentText("You are not the owner"));
            }
